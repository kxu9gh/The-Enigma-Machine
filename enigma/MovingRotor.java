...
public class MovingRotor extends Rotor {
    ...
    public boolean rotates() {
        // if(order >= mach.numRotors() - mach.numPawls() + 1){        //when the order of rotor is in the range of moving rotor
        //     return true;
        // }
        // return false; // FIXME - How do we know whether this Rotor should rotate?
        //BRANCHING NOTE: doesn't matter whether the order; moving rotors rotate.
        return true;
    }

    @Override // Use this special tag when updating the behavior of a method this class inherits from Rotor
    public boolean atNotch() {
        if(_notches.length() == 2){
            if(_posn == alphabet().toInt(_notches.charAt(0)) || _posn == alphabet().toInt(_notches.charAt(1))){
                return true;
            }
        }else{
            if(_posn == alphabet().toInt(_notches.charAt(0))){
                return true;
            }
        }
