...
public class Rotor {
    ...
    public int setting() {
        return _posn; // FIXME - How do we keep track of my current position?
    }
    ...
    public int convertForward(int p) {
        ...
        return _permutation.wrap(_permutation.permute(p + _posn) - _posn);  // FIXME - How do we permute the index P, taking into account my current position?
    }

    /** Return the conversion of C (an integer in the range 0..size()-1)
     *  according to the inverse of my permutation. */
    public int convertBackward(int c) {
        return _permutation.wrap(_permutation.invert(c + _posn) - _posn);  // FIXME - How do we invert the index E, taking into account my current position?
    }
